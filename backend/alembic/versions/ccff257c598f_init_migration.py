...
Revision ID: ccff257c598f
Revises: 5d6b3180c9e3
Create Date: 2025-11-25 20:44:00.866329
...
revision: str = 'ccff257c598f'
down_revision: Union[str, Sequence[str], None] = '5d6b3180c9e3'
...
def upgrade() -> None:
    ...
    op.execute("DROP TABLE IF EXISTS behave_photos CASCADE;")
    op.execute("DROP TABLE IF EXISTS activity_templates CASCADE;")
    op.execute("DROP TABLE IF EXISTS preset_tag_tags CASCADE;")
    op.execute("DROP TABLE IF EXISTS behave_photos CASCADE;")
    op.execute("DROP TABLE IF EXISTS energy_levels CASCADE;")
    op.execute("DROP TABLE IF EXISTS tags CASCADE;")
    op.execute("DROP TABLE IF EXISTS letter CASCADE;")
    op.execute("DROP TABLE IF EXISTS behave CASCADE;")
    op.execute("DROP TABLE IF EXISTS users CASCADE;")
    op.execute("DROP TABLE IF EXISTS activity_templates CASCADE;")
    op.execute("DROP TABLE IF EXISTS behave_tag_tags CASCADE;")
    op.execute("DROP TABLE IF EXISTS behave_tags CASCADE;")
    op.execute("DROP TABLE IF EXISTS preset_tags CASCADE;")
    op.execute("DROP TABLE IF EXISTS user_tags CASCADE;")
    op.execute("DROP TABLE IF EXISTS activities CASCADE;")
    op.execute("DROP TABLE IF EXISTS user_tag_tags CASCADE;")
...
def downgrade() -> None:
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['activity_id'], ['activities.id'], name='behave_activity_id_fkey'),
    sa.ForeignKeyConstraint(['after_energy_id'], ['energy_levels.id'], name='behave_after_energy_id_fkey'),
    sa.ForeignKeyConstraint(['before_energy_id'], ['energy_levels.id'], name='behave_before_energy_id_fkey'),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='behave_user_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='behave_pkey'),
    postgresql_ignore_search_path=False
    )
